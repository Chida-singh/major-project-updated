...
class YouTubeTranscriptExtractor {
  ...
  isTranscriptPanelOpen() {
    return (
      document.querySelector('ytd-transcript-renderer, #transcript, .transcript-container') !==
      null
    );
  }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'checkTranscriptAvailability') {
    const hasTranscriptButton =
      document.querySelector('[aria-label*="transcript" i], button[title*="transcript" i]') !==
      null;
