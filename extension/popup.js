...
document.getElementById('extractTranscript').addEventListener('click', async () => {
  ...
  if (!url.includes('youtube.com/watch')) {
    ...
    statusDiv.innerText =
      'Not a YouTube video page! Please navigate to a YouTube video.';
    return;
  }
    ...
    navigator.clipboard
      ...
      .then(() => {
        ...
        copyButton.style.background =
          'linear-gradient(135deg, var(--persian-green), var(--charcoal))';

        setTimeout(() => {
          copyButton.innerText = originalText;
          copyButton.style.background =
            'linear-gradient(135deg, var(--sandy-brown), var(--burnt-sienna))';
        }, 2000);
      })
      .catch((err) => {
        ...
        copyButton.style.background =
          'linear-gradient(135deg, var(--burnt-sienna), var(--sandy-brown))';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document
    ...
    .addEventListener('click', copyTranscriptToClipboard);
        ...
        chrome.storage.local.get(`transcript_${videoId}`, (result) => {
          ...
          if (cached && Date.now() - cached.timestamp < 3600000) {
            ...
            document.getElementById('status').innerText =
              'Found cached transcript (click Extract to refresh)';
